...
--
  ...
  CREATE INDEX i_folderexperimentlink_owner ON folderexperimentlink(owner_id);
  CREATE INDEX i_folderexperimentlink_group ON folderexperimentlink(group_id);
  CREATE INDEX i_FolderExperimentLink_parent ON folderexperimentlink(parent);
  CREATE INDEX i_FolderExperimentLink_child ON folderexperimentlink(child);
  ...
  CREATE INDEX i_folderinstrumentlink_owner ON folderinstrumentlink(owner_id);
  CREATE INDEX i_folderinstrumentlink_group ON folderinstrumentlink(group_id);
  CREATE INDEX i_FolderInstrumentLink_parent ON folderinstrumentlink(parent);
  CREATE INDEX i_FolderInstrumentLink_child ON folderinstrumentlink(child);
  CREATE INDEX i_folderplatelink_owner ON folderplatelink(owner_id);
  CREATE INDEX i_folderplatelink_group ON folderplatelink(group_id);
  CREATE INDEX i_FolderPlateLink_parent ON folderplatelink(parent);
  CREATE INDEX i_FolderPlateLink_child ON folderplatelink(child);
...
CREATE SEQUENCE seq_folderexperimentlink; INSERT INTO _lock_ids (name, id) SELECT 'seq_folderexperimentlink', nextval('_lock_seq');
CREATE SEQUENCE seq_folderimagelink; INSERT INTO _lock_ids (name, id) SELECT 'seq_folderimagelink', nextval('_lock_seq');
CREATE SEQUENCE seq_folderinstrumentlink; INSERT INTO _lock_ids (name, id) SELECT 'seq_folderinstrumentlink', nextval('_lock_seq');
CREATE SEQUENCE seq_folderplatelink; INSERT INTO _lock_ids (name, id) SELECT 'seq_folderplatelink', nextval('_lock_seq');
...
CREATE FUNCTION folder_config_map_entry_delete_trigger_function() RETURNS "trigger" AS '
BEGIN
    DELETE FROM folder_config
        WHERE folder_id = OLD.id;
    RETURN OLD;
END;'
LANGUAGE plpgsql;

CREATE TRIGGER folder_config_map_entry_delete_trigger
    BEFORE DELETE ON folder
    FOR EACH ROW
    EXECUTE PROCEDURE folder_config_map_entry_delete_trigger_function();
...
CREATE INDEX folder_config_name ON folder_config(name);
CREATE INDEX folder_config_value ON folder_config(value);
...
ALTER TABLE folder_config ALTER COLUMN name TYPE TEXT;
ALTER TABLE folder_config ALTER COLUMN value TYPE TEXT;
