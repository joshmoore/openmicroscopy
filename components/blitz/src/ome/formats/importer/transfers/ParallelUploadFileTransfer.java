 ...
 * @since 5.0
 */
public class ParallelUploadFileTransfer extends AbstractFileTransfer {
    ...
    public String transfer(TransferState state) throws IOException, ServerError {
        ...
        try {
            Producer producer = new Producer(out, state);
    ...
    class Producer implements Callable<Object> {
        ...
        public Producer(WritableByteChannel out, TransferState state) {
              this.out = out;
              file = state.getFile();
              buf = state.getBuffer();
        }
        ...
        public Object call() {
            ...
            try {
                ...
                ch = raf.getChannel();
                // In the consumer we're just using the buf as a length suggestion
